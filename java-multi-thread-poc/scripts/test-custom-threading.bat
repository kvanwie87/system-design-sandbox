@echo off
REM ============================================================
REM Test script for :custom-threading module (port 8081)
REM Sends sample payloads to all endpoints and displays responses.
REM
REM Prerequisites: the app must be running on port 8081
REM   gradlew.bat :custom-threading:bootRun
REM ============================================================
...
curl -s -w "\nHTTP Status: %%{http_code}\n" -X POST "%BASE_URL%/notifications/send" -H "Content-Type: application/json" -d "{\"message\": \"Hello from custom-threading test!\"}"
echo.
